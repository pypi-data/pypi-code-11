        ...
        if selectType == COUNT_SELECT:
            ...
            selectClause = expression.select(
                [table.c.member.label('subject'),
                 expression.literal(str(RDF.type) if PY3 else str(RDF.type)).label('predicate'),
...
class TermType(types.TypeDecorator):
    ...
    def process_bind_param(self, value, dialect):
        ...
        if isinstance(value, (QuotedGraph, Graph)):
            return str(value.identifier) if PY3 else str(value.identifier)
        elif isinstance(value, Node):
            return str(value) if PY3 else str(value)
        else:
            return value
...
class SQLAlchemy(Store, SQLGenerator):
    ...
    def prefix(self, namespace):
        ...
        with self.engine.connect() as connection:
            ...
            namespace = str(namespace) if PY3 else str(namespace)
    ...
    def namespace(self, prefix):
        """Namespace."""
        res = None
        prefix_val = str(prefix) if PY3 else str(prefix)
        try:
            with self.engine.connect() as connection:
                nb_table = self.tables['namespace_binds']
