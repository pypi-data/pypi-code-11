...
class Wintx(object):
  ...
  def checkQueryDict(self, query, restricted_columns=None):
    ...
    if( restricted_columns is None ):
      ...
      raise WintxDatabaseError('Restricted query parameter list provided is not a list or tuple.')

    for column in query:
      if( column in restricted_columns ):
        raise WintxDatabaseError('Parameter \'%s\' is restricted for this query.' % column)
      if( column == 'latitude' or column == 'longitude' or column == 'level' ):
        if( type(query[column]) is not type(1) and
          ...
          raise WintxDatabaseError('Invalid query comparison operand/operation(s) provided for parameter \'%s\'.' % column)
        if( type(query[column]) is type({}) ):
          for comparison in query[column]:
            if( comparison not in comparison_operands ):
              raise WintxDatabaseError('Invalid query comparison operation \'%s\' provided for parameter \'%s\'.' % (comparison, column))
            if( type(query[column][comparison]) is not type(1.1) and
                type(query[column][comparison]) is not type(1) ):
              raise WintxDatabaseError('Invalid query operand provided for the comparison operation \'%s\' under the parameter \'%s\'.' % (comparison, column))
        ...
        if( type(query[column]) is not type('') and
            ...
            type(query[column]) is not type([]) ):
          raise WintxDatabaseError('Invalid query comparison operation(s) provided for parameter \'%s\'.' % column)
        if( type(query[column]) is type([]) ):
          for parameter in query[column]:
            if( type(parameter) is not type('') and
                type(parameter) is not type(u'') ):
              raise WintxDatabaseError('Invalid query comparison operand \'%s\' provided for parameter \'%s\'.' % (query[column], column))
      elif( column == 'time' ):
        ...
        if( type(query[column]) is type({}) ):
          for comparison in query[column]:
            ...
            if( type(query[column][comparison]) is not type(datetime.now()) ):
              raise WintxDatabaseError('Invalid query comparison operand \'%s\' provided for parameter \'%s\'.' % (comparison, column))
      else:
        raise WintxDatabaseError('Unknown query parameter \'%s\' found.' % column)
    ...
    for key in record:
      if( key == 'varname' ):
        if( type(record[key]) is not type('') and type(record[key]) is not type(u'') ):
          raise WintxDatabaseError('The variable name is not a string in the record dictionary.')
        else:
          pass
      elif( key == 'datatype' ):
        if( type(record[key]) is not type('') and type(record[key]) is not type(u'') ):
          raise WintxDatabaseError('The dataset name is not a string in the record dictionary.')
        else:
          pass
      elif( key == 'leveltype' ):
        if( type(record[key]) is not type('') and type(record[key]) is not type(u'') ):
          raise WintxDatabaseError('The level type is not a string in the record dictionary.')
        else:
          pass
      elif( key == 'level' ):
        if( type(record[key]) is not type(1) and type(record[key]) is not type(1.1) ):
          raise WintxDatbaseError('The level is not a numerical value in the record dictionary.')
        else:
          pass
      elif( key == 'latitude' ):
        if( type(record[key]) is not type(1) and type(record[key]) is not type(1.1) ):
          raise WintxDatbaseError('The latitude is not a numerical value in the record dictionary.')
        else:
          pass
      elif( key == 'longitude' ):
        if( type(record[key]) is not type(1) and type(record[key]) is not type(1.1) ):
          raise WintxDatbaseError('The longitude is not a numerical value in the record dictionary.')
        else:
          pass
      elif( key == 'value' ):
        if( type(record[key]) is not type(1) and type(record[key]) is not type(1.1) ):
          raise WintxDatbaseError('The value is not a numerical value in the record dictionary.')
        else:
          pass
      elif( key == 'time' ):
        if( type(record[key]) is not type(datetime.now()) ):
          raise WintxDatabaseError('The time is not a datetime object in the record dictionary.')
        else:
          pass
      else:
        raise WintxDatabaseError('The item \'%s\' in the record dictionary is not a valid entry.' % key)
  ...
  def __prepareWhere__(self, query_dict, restricted=None):
    ...
    for key in query_dict:
      ...
      if( type(query_dict[key]) == type({}) ):
        ...
        query = "%s %s IN ('%s')" % (query, column_name, "', '".join(query_dict[key]))
      else:
